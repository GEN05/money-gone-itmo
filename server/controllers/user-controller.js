...
class UserController {
  ...
  async addTransaction(req, res, next) {
    try {
      ...
      const userTransactions = await userService.addTransaction(
        req.user.id,
        date,
        category,
        value
  ...
  async deleteTransaction(req, res, next) {
    try {
      const { id } = req.body;
      const userTransactions = await userService.deleteTransaction(
        req.user.id,
        id
      );
      return await res.json(userTransactions);
  ...
  async addTransactionsFromBank(req, res, next) {
    try {
      ...
      const userTransactionsFromBank =
        await userService.addTransactionsFromBank(req.user.id, trnsList);
