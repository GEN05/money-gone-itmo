...
export function trnsFromLastMonth(trns: Transaction[], trnsFromBank: Transaction[]) {
    // last updated month
    const maxDate = Math.max(...trns.map(t => t.date), ...trnsFromBank.map(t => t.date));
    // first day of last updated month
    const firstDayOfMonth = dateToYYYYMMDD(new Date(maxDate)).split('-').splice(0, 2).join('-') + '-01';
    // expenses for the last updated month
    const cashLastMonth: Transaction[] = trns.filter(t => (dateToYYYYMMDD(new Date(t.date)) >= firstDayOfMonth));
    const cardLastMonth: Transaction[] = trnsFromBank.filter(t => (dateToYYYYMMDD(new Date(t.date)) >= firstDayOfMonth));
