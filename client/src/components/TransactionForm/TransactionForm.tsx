...
const TransactionForm: FC<TransactionFormProps> = ({
  ...
  const getTrnsFormBank = () => {
    ...
    const parseTrnsList = (parsedCSV: (string | undefined)[][]): TrnsType[] => {
      ...
      const currencyIndex = parsedCSV[0].findIndex(
        (t) => t === "Валюта операции"
      );
      ...
      const parseTrns = (trns: TrnsRawType): TrnsType | null => {
        ...
        const shopping = [
          ...
          "Косметика",
          "Красота",
          "Одежда, обувь, ювелирные изделия и часы",
          "Одежда, обувь",
          "Спорттовары",
          "Сувениры",
        ];
        ...
        else if (trns.currency === "EUR")
          validValue = Math.abs(Number(trns.value)) * 1.1;
        else if (trns.currency === "RUB")
          validValue = Math.abs(Number(trns.value)) / 78;
        else validValue = NaN;
      ...
      return validTrns
        .map(parseTrns)
        .filter((trns): trns is TrnsType => trns !== null)
        .reverse();
    };
