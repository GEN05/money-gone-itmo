...
const TransactionList: FC<TransactionListProps> = ({ trnsList }) => {
  ...
  return (
    <div className="transactions">
        ...
        [...store.user.transactions].reverse().map((trns) => {
          return (
            <div
              ...
              className={
                "transaction" +
                (activeTrns === trns.id ? " cursor-disabled" : "")
              }
              tabIndex={1}
            ...
            >
              ...
              <div className="transaction_info">
                ...
                <span>{dateHumanReadableFull(new Date(trns.date))}</span>
              </div>
      ...
      {trnsList === "bank" &&
        [...store.user.transactionsFromBank].reverse().map((trns) => {
          return (
            <div
              id={trns.id}
              key={trns.id}
            ...
            >
              <img
                className="transaction_category_img"
                src={
                  categoryTinkoffImgs[trns.category] || categoryImgs["other"]
                }
                alt={trns.category}
              />
